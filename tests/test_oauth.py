...
from lothc import HTTPClient, HTTPResponseError, OAuthProvider, SyncHTTPClient, SyncOAuthProvider
...
async def test_token_inside_leeway_window_is_minted_again_without_refresh_token(
    ...
    key = str(uuid4())
    provider = OAuthProvider(
        token_url=f"{base_url}oauth/token?key={key}&expires_in=1000",
        client_id="client-id",
        client_secret="client-secret",
        refresh_leeway=2000.0,
    )
...
async def test_token_inside_leeway_window_is_refreshed_with_refresh_token(
    ...
    key = str(uuid4())
    provider = OAuthProvider(
        token_url=f"{base_url}oauth/token?key={key}&expires_in=1000&with_refresh=1",
        client_id="client-id",
        client_secret="client-secret",
        refresh_leeway=2000.0,
    )
...
async def test_rejected_refresh_falls_back_to_minting(base_url: str, client: HTTPClient) -> None:
    ...
    provider = OAuthProvider(
        token_url=(
            f"{base_url}oauth/token?key={key}&expires_in=1000&with_refresh=1&refresh_fails=1"
        ),
        client_id="client-id",
        client_secret="client-secret",
        refresh_leeway=2000.0,
...
async def test_custom_models_refresh_with_token_refresh_request(
    ...
    key = str(uuid4())
    provider = OAuthProvider(
        token_url=f"{base_url}oauth/token-custom?key={key}&expires_in=1000&with_refresh=1",
        client_id="client-id",
        client_secret="client-secret",
        token_request=_TokenRequest,
...
async def test_token_endpoint_server_error_propagates(base_url: str) -> None:
    provider = OAuthProvider(
    ...
    async with HTTPClient.build(base_url=base_url, bearer_auth=provider) as api:
        with pytest.raises(HTTPResponseError) as error_info:
            await _bearer(api)

    assert error_info.value.status == 500


def test_sync_token_endpoint_server_error_propagates(base_url: str) -> None:
    provider = SyncOAuthProvider(
        token_url=f"{base_url}oauth/token-boom?key={uuid4()}",
        client_id="client-id",
        client_secret="client-secret",
    ...
    with (
        SyncHTTPClient.build(base_url=base_url, bearer_auth=provider) as api,
        pytest.raises(HTTPResponseError) as error_info,
    ):
        _sync_bearer(api)

...
Covers all four decode targets `get()` supports: raw `bytes` (default), plain `dict`, a pydantic
`BaseModel`, and a msgspec `Struct`. pydantic/msgspec are both optional extras, not `lothc`'s own
hard dependency, so a real isolated `asv run` build only has them if `asv.conf.json`'s `matrix.req`
says so.
...
class VerbSuite:
    ...
    def time_get_typed(self) -> None:
        self._client.get("items/7", response_data_type=dict)
